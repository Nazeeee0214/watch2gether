...
import { Users, Link as LinkIcon, Check, Monitor } from 'lucide-react';
...
function RoomContent({ roomId }: { roomId: string }) {
  ...
  const { joinRoom, roomState, isHost, socket, isScreenSharing, startScreenShare, stopScreenShare } = useRoom();
  ...
  useEffect(() => {
    if (roomId) {
      joinRoom(roomId, username, isCreatorParam);
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                {isHost && (
                   <div className="flex flex-col sm:flex-row gap-3">
                     ...
                     <button
                       ...
                       className="px-6 py-3 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 font-medium rounded-xl transition-all shadow-lg shadow-red-500/5 whitespace-nowrap"
                     >
                       Clear Video
                     </button>
                   </div>
          ...
          <div className="bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-3xl h-fit">
            <div className="flex items-center gap-2 mb-6 text-white/80">
              <Users size={18} />
              <h2 className="font-semibold">Participants ({roomState.users.length})</h2>
            </div>
              ...
              {roomState.users.map((user) => (
                <li key={user.userId} className="flex items-center justify-between p-3 rounded-xl bg-black/40 border border-white/5">
                  ...
                  {user.userId === roomState.hostId && (
                    <span className="text-xs px-2 py-1 bg-purple-500/10 text-purple-400 rounded-md font-medium whitespace-nowrap">
                      Host
                    </span>
                  )}
                </li>
